...
- G0–G3 纯软件门：PASS。
- G0–G3 已通过 `81657c4` 合入 `origin/main@39e8a92`；当前续跑分支为 `codex/mycobot-experiment-main-sync-20260714`，直接基于该云端主线提交创建。
- G4 正式 SoC/PNR/部署、烧录、J52、UART2 和真实机械臂：NO-GO。
- 当前制品全部是 `NOT_FOR_FLASH`，不能直接烧录。

## 两条路线必须分开

- **PC 实验路线**：只用于 180°点位示教、外部基准记录和无夹爪单段诊断；按 `mycobot_pc_experiment_continuation_plan_20260714.md` 执行。
- **板上路线**：从本文件 G4 开始，目标是正式 SoC/PNR、断臂板上模拟、UART2 无臂回环和后续只读/动作门。
- PC 点位或动作日志不能替代 G4–G11；单摄候选工程也必须先完成自身 M0 板级复现，不能作为机械臂放行证据。
...
本说明不授权烧录、接线或机械臂动作。继续 PC 实验时先执行独立续跑计划；继续板上路线时，仍须由用户明确回复是否具备 G4 条件。
