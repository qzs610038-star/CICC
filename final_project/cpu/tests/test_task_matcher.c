...
/* ---- MODE_2: mixed-shape pool, color+shape match, size ignored ---- */

static void test_mode2_color_shape_match(void)
{
    TEST("mode2: target color+shape match → GRAB");
    task_matcher_init();
...
static void test_mode2_size_ignored(void)
{
    TEST("mode2: size differs but color+shape match → GRAB");
    task_matcher_init();
...
static void test_mode2_size_unavailable_still_matches(void)
{
    TEST("mode2: size unavailable but color+shape match → GRAB");
    task_matcher_init();
    task_target_t t = make_target_ex(COLOR_RED, SHAPE_CUBE, 20, 0, TASK_MODE_2);
    task_matcher_set_target_ex(&t);
...
static void test_same_target_reapply_preserves_lock(void)
{
    TEST("round: same-target reapply is idempotent and cannot rearm GRAB");
    task_matcher_init();
    task_target_t t = make_target_ex(COLOR_RED, SHAPE_CUBE, 20, 0, TASK_MODE_2);
    task_matcher_set_target_ex(&t);
    ...
    /* Simulate main() polling the same target register on the next video frame. */
    task_matcher_set_target_ex(&t);
    CHECK_EQ(task_matcher_get_round_state(), ROUND_GRAB_REQUESTED);
    CHECK_EQ(task_matcher_evaluate(&obs, 30, 40), MATCH_ACTION_NONE);

    /* An explicit next_round is required before the same target can arm again. */
    task_matcher_next_round();
    task_matcher_set_target_ex(&t);
    CHECK_EQ(task_matcher_evaluate(&obs, 50, 60), MATCH_ACTION_GRAB);
    PASS();
}
...
int main(void)
{
    ...
    test_mode2_color_shape_match();
    test_mode2_size_ignored();
    test_mode2_size_unavailable_still_matches();
    ...
    test_same_target_reapply_preserves_lock();
