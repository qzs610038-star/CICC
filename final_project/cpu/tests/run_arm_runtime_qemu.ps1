Param(
    ...
    [string]$QemuPath = $env:EFINITY_RISCV_QEMU
)

$ErrorActionPreference = 'Stop'
$root = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path
$toolchain = $ToolchainPath
if (-not $toolchain -and $env:EFINITY_HOME) {
    $candidate = Join-Path $env:EFINITY_HOME 'toolchain'
    if (Test-Path -LiteralPath $candidate) { $toolchain = $candidate }
}
if (-not $toolchain) {
...
if (-not $QemuPath -and $env:EFINITY_HOME) {
    $candidate = Join-Path $env:EFINITY_HOME 'qemu\qemu-system-riscv32.exe'
    if (Test-Path -LiteralPath $candidate) { $QemuPath = $candidate }
}
if (-not $QemuPath) {
    $qemuCommand = Get-Command 'qemu-system-riscv32.exe' -ErrorAction SilentlyContinue
    if ($qemuCommand) { $QemuPath = $qemuCommand.Source }
}
if (-not $QemuPath) {
    throw 'QEMU not found. Pass -QemuPath or set EFINITY_RISCV_QEMU.'
}
$gcc = Join-Path $toolchain 'bin\riscv-none-embed-gcc.exe'
$qemu = [IO.Path]::GetFullPath($QemuPath)
