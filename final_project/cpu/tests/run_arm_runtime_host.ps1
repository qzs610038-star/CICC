Param(
    [string]$VcVarsPath = $env:VCVARS64_PATH
)
...
$vcvars = $VcVarsPath
if (-not $vcvars) {
    $vcvars = Get-ChildItem -Path (Join-Path $env:ProgramFiles 'Microsoft Visual Studio\2022\*\VC\Auxiliary\Build\vcvars64.bat') -ErrorAction SilentlyContinue |
        Select-Object -First 1 -ExpandProperty FullName
}
if (-not $vcvars) {
    throw 'MSVC environment not found. Pass -VcVarsPath or set VCVARS64_PATH.'
}
