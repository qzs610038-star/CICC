...
static void clear_result(competition_contract_t *contract)
{
    ...
    contract->result.round_state = contract->round.state;
...
{
    contract->result.event_seq = contract->round.event_seq;
    contract->result.round_state = contract->round.state;
    contract->result.size_state = contract->size_state;
                               ...
                               size_state_t size_state)
{
    if (contract != 0) {
        ...
        round_controller_init(&contract->round);
...
{
    if (contract == 0 || contract->round.state == ROUND_STATE_WAIT_OBSERVATION ||
        contract->round.state == ROUND_STATE_WAIT_ACK) {
        return -1;
    }
                                      ...
                                      uint32_t now_ms, uint32_t timeout_ms)
{
    ...
    case COMP_EVENT_PLACE:
        if (!contract->active.valid ||
            round_controller_start(&contract->round, &contract->active.target,
                                   event->event_seq, now_ms, timeout_ms) != 0) {
            return -1;
        }
        clear_result(contract);
    ...
    case COMP_EVENT_REMOVE:
        if (contract->round.state != ROUND_STATE_COMPLETE &&
            contract->round.state != ROUND_STATE_ABANDONED &&
            contract->round.state != ROUND_STATE_TIMEOUT) {
            return -1;
        }
        round_controller_soft_reset(&contract->round);
        clear_result(contract);
        break;
    case COMP_EVENT_ABANDON:
        if (round_controller_abandon(&contract->round) != 0) {
            return -1;
        }
                                 ...
                                 uint32_t now_ms)
{
    ...
    if (contract->size_state == SIZE_STATE_UNAVAILABLE &&
        task_requires_size(contract->round.target.mode)) {
    ...
    evaluation = round_controller_observe(&contract->round, observation, now_ms);
    write_result(contract, observation, evaluation);
    return 0;
}
...
int competition_contract_ack_result(competition_contract_t *contract,
                                    uint16_t event_seq)
{
    if (contract == 0 || round_controller_ack(&contract->round, event_seq) != 0) {
        return -1;
    }
    contract->result.round_state = contract->round.state;
    return 0;
...
int competition_contract_tick(competition_contract_t *contract, uint32_t now_ms)
{
    ...
    if (round_controller_tick(&contract->round, now_ms) > 0) {
        write_result(contract, 0, (competition_evaluation_t){
