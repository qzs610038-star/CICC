...
/* 8-bit event_seq 新旧判定（支持回绕）。
 * delta = (uint8_t)(seq - last)：
 *   delta==0        → 同一序号，视为重复；
 *   1..127          → 向前的新序号，接受（含回绕，如 255→0 delta=1）；
 *   128..255        → 过期/倒退序号，拒绝（如 8→7 delta=255）。
 * 首个事件由调用方的 have_event_seq==0 单独放行。 */
static int seq_is_newer(uint8_t seq, uint8_t last)
{
