...
本指南是 [主图：项目数据链路与三人分工](file:///d:/第十届集创赛-雄芯院材料/learning_guides/接口对齐与数据链路学习/1_project_data_link_and_roles_guide.md#2-项目整体数据传输链路与队员分工图示) 的**局部放大图**。它只展开主图中的三条 qzs 责任支路：
...
权威事实以 [CURRENT_STATE.md](file:///d:/第十届集创赛-雄芯院材料/CURRENT_STATE.md)、[F1 接口对齐总账](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/F1_INTERFACE_ALIGNMENT_DRAFT.md) 与 [单摄特征快照契约](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/single_camera_feature_contract.md) 为准。
...
flowchart TB
    ...
    subgraph DETAIL_LOGIC["【qzs 负责的 I4 细节逻辑与 Staging-Commit 握手机制】"]
        ...
        PAYLOAD[["OSD 暂存寄存器 (Staging)<br/>round_id、frame_id、config_revision<br/>color、shape、decision、reason、arm_enabled=0"]]
        COMMIT[["提交写入：CFG_COMMIT<br/>result_valid=1 + result_round_id"]]

        LATCH["FPGA OSD 整体锁存<br/>(仅在 VSYNC 场消隐边界同步)"]
        OSD["FPGA OSD 渲染器<br/>(读取 Active 寄存器)"]
        HDMI["HDMI 显示可解释文本"]
    end
...
*   **Staging 暂存区 (PAYLOAD)**：回写协议中为防止显示字符“写一个字亮一个字”而设计的缓冲寄存器。
*   **LATCH 整体锁存**：在场消隐消退边界，OSD 从机一次性同步加载所有 staging 的结果，保证字符显示的瞬间原子对齐。
*   **result_valid (COMMIT)**：原子回写提交标识。置 1 时 FPGA 才会读取暂存数据更新显示，置 0 时可清屏。
...
4.  **誊抄与广播（Latch & Render）**：等到下课课间操操场没人时（VSYNC 场消隐消退），FPGA OSD 渲染器这个“值日生”会以迅雷不及掩耳之势把草稿纸内容誊抄在黑板上（Active Latch），瞬间更新 HDMI 大屏幕，确保全场在操场回来后一眼看到的就是最准确、不闪烁的战报。
...
flowchart TB
    ...
    subgraph DETAIL_LOGIC["【qzs 负责的 I5 物理隔离与软件硬互锁双重防爆细致链路】"]
        ...
        J52_DIS["物理底锁：物理拔除信号跳线<br/>(J52 UART2 链路物理悬空)"]
        ARM_LOCK["myCobot 280 物理电机锁死<br/>(保持安全姿态且无暴冲风险)"]
    end
...
*   **物理隔离**：拔除串口控制线，使得板卡即使产生非预期的电平震荡，也没有物理介质流向机械臂。
...
1.  **软件首锁（大锁栓）**：大脑（CPU）在决策判定目标命中时，由于 F1 演习限制，强制在命令中写入“留在原位不发射（ARM_DISABLED）”，并且完全屏蔽了发射控制软件（不初始化 UART2 驱动），机械臂底端接收不到任何控制包。
2.  **物理底锁（拔电缆）**：万一代码由于静电或者初始化时产生乱码，意外发生了串口抖动（发射钥匙震荡），qzs 在物理上直接拔掉了核弹发射电缆（J52 串口排线断开，物理隔离）。电平无法传导，机械臂锁死在安全姿态，做到物理与软件的双保险，杜绝暴冲。
...
I5 接口是系统的**“物理动能控制器”**。机械臂底座上跑着 1M 波特率的高速串口，一旦收到半包、乱码或者非预期脉冲，其气动夹爪或多轴电机会以极高速度旋转，极其危险。因此我们在 F1 阶段将其定义为“彻底阻断（BLOCKED）”状态。在未通过 G1 级的所有心跳与总线测试前，禁止机械臂信号线连线，更禁止从代码里发送任何命令帧，保证人身与设备绝对安全。
