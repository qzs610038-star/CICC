...
- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 UART0 Hello 板卡归还后的 FTDI 前置项复核）
  - 适用范围：仅 `competition_project_single_camera/` 隔离 CPU Hello；不关闭 `final_project/` A0/G4，不开放 UART2/J52/myCobot。
  - 主线核验：当前本地 `main=118be9da1f3cdf63c34624e7ef9cf3b8f9171335`，包含 LBS/libaoxun688 合并 `041116c`。该合并相对第一父提交只改 `.gitignore` 的输出忽略项，未改变本 Gate 的 USER2、UART0、BSP、XML、SDC 或 RTL 契约。
  - FTDI 前置项：只读采集器已以 exit 0 新建 `m2_ftdi_live_verify_20260716_152234.json`；FTDI `0403:6011` A/C/D 通道 COM3、COM7、COM8 均为 Started。初次 HOLD 已定位为采集器未接受 Windows `VID_0403+PID_6011` 拼写的解析缺陷，修复为 `[&+]` 后用 PnP 与 pyserial 双重核验。采集全程 `serial_port_opened=false`、`uart_bytes_sent=0`、`programmer_invoked=false`、`flash_operation_invoked=false`。因此“板卡 FTDI 已枚举”前置项已关闭；UART0 通道映射、USER2、CPU 取指和横幅/回显仍为 `NOT VERIFIED`。
  - 机械臂隔离：用户已确认机械臂端口物理断开。COM11 仍为未归属 CH340 `1A86:7523`，VID/PID 不足以重新归属为 myCobot；未打开该端口，且它不得用于本 Gate。
  - 制品身份预检：操作员已对 `mem_test.bit` 与 Hello ELF 运行 SHA-256，并回传 `2EA4AD287CCC6BFBF113E718B59EF6F5807222AFE1ECE3D55BD1E24D37FB8347` / `C99FD39DB437409A63A6061CD29698B5B60099B9E24A77B155B871E169BF5DA5`。Codex 对同一现存文件独立重算 exit 0，一致。因此仅可进入操作卡的“易失性 FPGA 配置”层；这不表示 FPGA 已配置或 USER2/UART0 已通过。
  - 下一门：仅按 `m2_uart0_cpu_hello_board_operator_card_20260716.md` 做匹配 bitstream 的易失性配置、`JTAG_USER2` 的 `0xF9000000` Hello ELF 下载和 115200 8N1 横幅/一次 `x` 回显。任一层失败即停止，禁止 USER1、Flash、DDR、UART2/J52 或 myCobot。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_ftdi_live_verify_20260716_152234.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_operator_hash_preflight_20260716.md`、`competition_project_single_camera/tools/capture_m2_ftdi_preflight.ps1`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_cpu_hello_preflight_20260716.md`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_cpu_hello_board_operator_card_20260716.md`。
  - 失效条件：FTDI 枚举、bit/ELF hash、JTAG target、UART0 通道、机械臂隔离或任一板级现象发生变化；届时重新建立证据，不沿用本条。

- 日期：2026-07-16，来源 Agent：操作员烧录截图 / Codex 制品身份复核（单摄 M2 UART0 Hello）
  - 适用范围：仅 `competition_project_single_camera/` 隔离 CPU Hello；不关闭 `final_project/` A0/G4，不开放 UART2/J52/myCobot。
  - 最新结论：操作员的 Efinity Programmer 截图确认 FTDI `0403:6011`（`FTBI7G42`）、`TJ375N529`、JTAG、器件 ID `0x006A0EF3`、6 MHz、传输完成及 `Device is in user mode!`。但是截图实际加载的是本机 ASCII 暂存目录中遗留的 `outflow/mem_test.bit`；Codex 新鲜重算该文件 SHA-256 为 `9515FAC58CBE4DC07ADED28B6038C64485989C6BE650D09E5C07F6D5C4F1A169`，不等于唯一批准的本批 `2EA4AD287CCC6BFBF113E718B59EF6F5807222AFE1ECE3D55BD1E24D37FB8347`。故裁定为 `CONFIG_TRANSPORT_PASS / BATCH_IDENTITY_NOT_ACCEPTED`，不得作为候选 M2 配置完成证据。
  - 根因与恢复：手工目录同步器故意排除 `outflow`/`outflow_*` 并保留目标已有生成物，不能保证其旧 `outflow/mem_test.bit` 被更新。允许直接将批准 bitstream 复制到操作员指定的专属 ASCII 暂存批次目录，并对源与目标都核验完整 SHA-256 为 `2EA4...B8347` 后，仅做一次易失性 JTAG 重新配置。不可用文件大小或 Programmer 的短 `Checksum` 代替 SHA-256。
  - 下一门：重新配置时必须在 GUI 中可见该专属 ASCII 路径，成功页须保留 `Device is in user mode!`。在 Codex 复核该新证据前，`JTAG_USER2`、ELF `0xF9000000` 下载、UART0 及任何 UART2/J52/myCobot 操作继续禁止。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_programmer_attempt_review_20260716.md`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_cpu_hello_board_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：操作员第二次烧录截图 / Codex 新鲜制品复核（单摄 M2 UART0 Hello）
  - 适用范围：仅 `competition_project_single_camera/` 隔离 CPU Hello；不关闭 `final_project/` A0/G4，不开放 UART2/J52/myCobot。
  - 最新结论：操作员第二次 Programmer 截图显示实际使用操作员指定的专属 ASCII 暂存批次镜像，FTDI `0403:6011`/`FTBI7G42`、`TJ375N529`、JTAG、`0x006A0EF3`、6 MHz、配置完成及 `Device is in user mode!` 均与本 Gate 一致。截图文件 SHA-256 为 `50F819E0B92288CE2B2EA419CAAB9A4F03AF567AAB282136F750A21FA60F109B`。Codex 对该 ASCII 镜像与仓库批准 bitstream 新鲜重算，二者完整 SHA-256 均为 `2EA4AD287CCC6BFBF113E718B59EF6F5807222AFE1ECE3D55BD1E24D37FB8347`、大小均为 `11,843,718 B`。因此 `VOLATILE_BITSTREAM_CONFIG_PASS`；此前 `9515...A169` 的旧 `outflow/mem_test.bit` 误选记录不再代表当前 FPGA 配置。
  - 下一门：只按 `m2_uart0_user2_ram_download_operator_card_20260716.md` 建立 QCRV32 硬件调试会话：固定 Hello ELF（SHA-256 `C99FD39DB437409A63A6061CD29698B5B60099B9E24A77B155B871E169BF5DA5`）经 `JTAG_USER2` 下载到 `0xF9000000` 片上 RAM，并停在 `_start` 或 `main` 取证。此时不得 Resume、打开 UART 或发送字节；USER1、Flash、外部 DDR、UART2/J52 和机械臂继续禁止。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_volatile_config_acceptance_20260716.md`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 USER2 调试配置离线审计）
  - 适用范围：仅隔离 CPU Hello 的 USER TAP 身份；不表示 USER2 已连通、CPU 已执行或 UART0 已输出。
  - 最新结论：当前 Hard SoC 的 `settings.json` 与 `hard_ip_args.ini` 都是 `INTF_JTAG_TAP_SEL=9`；Efinity 2025.2 官方 IP 枚举与生成器明确 `9 → JTAG_USER2`，并且当前 `mem_test.peri.xml` 中 `soc_jtag_inst1` 的 `jtag_def` 也是 `JTAG_USER2`。因此 USER2 是本 Gate 的唯一允许选择。仓库提交的最小 BSP 未生成 `openocd/`、`debug_profile_hard.cfg` 或 `.launch`；后续 ASCII 手工目录已由受控生成器补齐，详见本文件后续“硬 TAP 配置链纠偏”条目。不得把 Efinity `Ti375C529_devkit` 的 `INTF_JTAG_TAP_SEL=8`（官方映射为 USER1）样例复制过来。
  - 下一门：本条的旧 UI 表述已由后续“硬 TAP 配置链纠偏与运行时 PC 门”条目收紧：只接受官方 TI/Titanium 硬 TAP 模板链，且下载后必须通过 PC 范围取证；不授权 Resume、串口或 UART2/J52/myCobot。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_user2_profile_audit_20260716.md`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 USER2 ELF 镜像只读预检器）
  - 适用范围：在 USER2/OpenOCD/GDB 开始前固定 Hello ELF 的 ASCII 镜像身份；不配置 FPGA、不启动调试器、不打开串口。
  - 最新结论：新增 `competition_project_single_camera/tools/capture_m2_user2_elf_preflight.ps1`。它只核验仓库 Hello ELF 与操作员显式传入的 ASCII 暂存镜像的 SHA-256、记录当前 Git HEAD，并将证据 JSON 写入 debug session evidence。零副作用字段明确为：串口未打开、UART 字节为 0、Programmer/OpenOCD/GDB/Flash 均未调用、未选择 USER TAP。自测中，固定仓库 ELF hash 为 `C99FD39DB437409A63A6061CD29698B5B60099B9E24A77B155B871E169BF5DA5`；在 ASCII ELF 尚未复制的当前状态，默认模式 exit 0 且明确 `HOLD_ELF_MIRROR_MISSING`，`-RequireMirror` 按设计 exit 2，两个 JSON 的硬件副作用字段均为零。操作卡现要求显式传入候选工程根与暂存目录；该修正未改变硬件或制品状态。
  - 下一门：操作员复制 ELF 后先运行采集器；仅出现 `ELF_MIRROR_READY_FOR_USER2_RAM_LOAD` 时才按 USER2 RAM 下载操作卡进入 IDE。采集器通过不表示 USER2、CPU 取指或 UART0 已通过。
  - 证据路径：`competition_project_single_camera/tools/capture_m2_user2_elf_preflight.ps1`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_elf_preflight_20260716_160707.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_elf_preflight_20260716_160718.json`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 完整 USER2 调试 BSP 生成器）
  - 适用范围：为 ASCII 手工目录补齐与当前 Hard SoC 参数一致的 OpenOCD/调试支持文件；不替代最小 Hello BSP 的构建身份，不配置 FPGA 或加载 CPU。
  - 最新结论：新增 `competition_project_single_camera/tools/prepare_m2_user2_debug_bsp.ps1`。脚本只允许写入仓库外、由操作员显式传入的 ASCII 暂存目录，读取仓库当前 `settings.json`，将其中 35 个单比特 Verilog 字面量仅在进程参数中归一化为官方生成器所需的 `0/1`，并使用 Efinity 2025.2 的 `sw_script.py` 生成完整 BSP。运行前若发现 RISC-V IDE/OpenOCD/GDB/jtag daemon 进程则 fail-closed。首次 dry-run exit 0；随后 `-Apply` exit 0，并已在 ASCII 手工目录生成完整 `openocd/` 支持。验收为 `USER2_DEBUG_BSP_READY`：`debug_profile_hard.cfg` 的 TAP=9、Device=`TJ375N529`，`debug_ti.cfg` 含 `riscv set_bscan_tunnel_ir 9` 且无未解析占位符；`debug_softTap.cfg` 的旧 `0x00001000` 入口只作为拒绝证据，禁止本 Gate 使用。最终哈希证据批次为：hard profile `498B4BF68D02452914F987C31AC0348E8576D501A6A3E2AC1DF7D67328EA2040`、允许的 `debug_ti.cfg` `60B754D1FC0D3F38B0939D8087A481627E0AC31C39CED2A43834A48BF0F60DE2`、禁止的 softTap `22C0F75743AB4B454794C072F7904DFECF20C96A1C49260B92F6050FAAB08332`、`external.cfg` `80B9328C72266C02C4BB8C376D3A9FDD3FF99F3438329843B6DF62884C1121F5`、`jtag_daemon.cfg` `DAC0F2648186800008E4D09753D7AF7D4CE249DEFF09940A0D38C0C0E54979A0`。全程未调用 Programmer/OpenOCD/GDB/串口/Flash。
  - 下一门：此处的 `USER2_DEBUG_BSP_READY` 是 v1 生成记录；已由后续 v2 `USER2_HARD_TAP_DEBUG_SUPPORT_READY_RUNTIME_PC_GATE_REQUIRED` 与运行时 PC 门覆盖。生成脚本通过仍不表示 CPU 已连接、取指或 UART0 已输出。
  - 证据路径：`competition_project_single_camera/tools/prepare_m2_user2_debug_bsp.ps1`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_161226.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_161239.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_161318.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_user2_profile_audit_20260716.md`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 USER2 Hello ELF ASCII 镜像已就绪）
  - 适用范围：只关闭 USER2 IDE 进入前的 ELF 文件身份前置项；不连接 CPU、不开串口、不执行 ELF。
  - 最新结论：在确认没有 RISC-V IDE/OpenOCD/GDB/jtag daemon 运行后，已将仓库固定 Hello ELF 复制到操作员指定的 ASCII 暂存批次目录。复制前后源/镜像完整 SHA-256 均为 `C99FD39DB437409A63A6061CD29698B5B60099B9E24A77B155B871E169BF5DA5`。随后 `capture_m2_user2_elf_preflight.ps1 -RequireMirror` exit 0，结果 `ELF_MIRROR_READY_FOR_USER2_RAM_LOAD`；其 JSON 断言串口未开、UART 字节为 0、Programmer/OpenOCD/GDB/Flash 都未调用。
  - 下一门：只剩操作员按 USER2/RAM 下载卡在 Efinity RISC-V IDE 中选择 `TJ375/QCRV32 + USER2 + Debug in RAM`，加载该 ASCII ELF 后停在 `_start` 或 `main` 取证；不得 Resume、打开 UART 或发送字节。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_elf_preflight_20260716_161436.json`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 USER2 硬 TAP 配置链纠偏与运行时 PC 门）
  - 适用范围：仅隔离 CPU Hello 的 IDE 下载前静态配置支持；不表示 USER2 已连接、ELF 已下载、CPU 已执行或 UART0 已输出，也不关闭 `final_project/` A0/G4。
  - 最新结论：已复核 Efinity RISC-V IDE 2025.2 官方 `ti.ftl`：其硬 TAP 链固定使用 `ftdi_ti.cfg + debug_ti.cfg`，并为 `Debug in RAM`、加载 image/symbols、不手设 PC、`stop at main`。此前把 `instr_addr=0x00001000` 仅归于 `debug_softTap.cfg` 的表述不完整：当前 `debug_ti.cfg` 也含该历史声明。新生成器/预检器 `m2_user2_debug_bsp_v2` 已在 `-Apply` 下 exit 0，结果为 `USER2_HARD_TAP_DEBUG_SUPPORT_READY_RUNTIME_PC_GATE_REQUIRED`；它验证 `TAP=9/USER2`、`TJ375N529`、`debug_ti` 的 bscan IR=9、`ftdi_ti` 的 `0403:6011/channel 1` 和官方模板链。静态文本未发现 `debug_ti.cfg`、`ftdi_ti.cfg` 或 `ti.ftl` 消费 `$instr_addr`，但此事实**不能证明**运行时 OpenOCD/CPU PC 行为。
  - 强制下一门：只按更新后的 `m2_uart0_user2_ram_download_operator_card_20260716.md` 使用官方 TI/Titanium 硬 TAP 模板、固定 ASCII ELF 与 RAM debug。加载后在任何 Resume、串口打开或 UART 字节之前，必须截图证明调试器 PC/反汇编处于 `0xF9000000..0xF9003FFF`；若显示 `0x00001000`、其它地址、USER1、SoftTap、External、Flash 或 DDR 行为，立即停止并回传日志。不得靠手改 cfg 消除风险。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_162132.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_user2_profile_audit_20260716.md`、`competition_project_single_camera/tools/prepare_m2_user2_debug_bsp.ps1`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 TJ375 CPUTAPID 静态纠偏）
  - 适用范围：仅隔离 CPU Hello 的 USER2 硬 TAP OpenOCD 会话参数；不重新配置 FPGA、不加载 ELF、不执行 CPU、不打开 UART，不关闭 `final_project/` A0/G4。
  - 最新结论：当前 `mem_test.xml` 目标为 `TJ375N529`，Efinity 2025.2 `device_db.xml`、`EFX_JTAG_CTRL.v` 与操作员 Programmer 实读均为 JTAG ID `0x006A0EF3`。但官方 Hard SoC `ftdi_ti.cfg` 默认 `CPUTAPID=0x006A0A79`，该 ID 属于 Ti375；`debug_ti.cfg` 将其用于 `jtag newtap -expected-id`，而 `ti.ftl` 没有自动覆盖。故此前“直接按 TI 模板连接”不足，必须先在模板的 Other options 第一行加会话级 `-c 'set CPUTAPID 0x006A0EF3'`，再保留 `ftdi_ti.cfg + debug_ti.cfg` 两行。`ftdi_ti.cfg` 已静态证明支持预定义变量覆盖。
  - 预检结论：更新后的 `prepare_m2_user2_debug_bsp.ps1 -Apply` exit 0，schema `m2_user2_debug_bsp_v3`、结果 `USER2_HARD_TAP_DEBUG_SUPPORT_READY_TJ375_CPUTAPID_OVERRIDE_AND_RUNTIME_PC_GATE_REQUIRED`。它同时核验 USER2/TAP=9、TJ375 ID、覆盖钩子和运行时 PC 门；证据中串口/Programmer/OpenOCD/GDB/Flash 副作用均为零。
  - 强制下一门：只按更新后的 USER2 RAM 下载操作卡，通过官方 TI/Titanium 硬 TAP 模板添加上述覆盖，加载固定 ELF 后、任何 Resume 或串口动作前，截图确认 PC/反汇编为 `0xF9000000..0xF9003FFF`。缺少覆盖、出现 `0x00001000`/其他 PC、USER1/SoftTap/External/Flash/DDR 或连接失败均 STOP；不得手改 cfg 或改用其他 ID。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_162733.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_user2_profile_audit_20260716.md`、`competition_project_single_camera/tools/prepare_m2_user2_debug_bsp.ps1`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 USER2 片上 RAM work area 静态纠偏）
  - 适用范围：仅隔离 CPU Hello 的 OpenOCD RAM 下载安全配置；不连接调试器、不加载或执行 ELF、不打开 UART，不关闭 `final_project/` A0/G4。
  - 最新结论：固定 Hello map 的 `_start=0xF9000000`、已用末端 `0xF9000A30`，而官方 `debug_ti.cfg` 初始为四个 hart 设定 `work-area=0xF9000000..0xF90003FF`，存在程序入口重叠。OpenOCD 的官方 work-area 语义允许批量目标内存写入且默认不备份，不能接受此重叠。受控生成器没有修改官方 cfg，而是在 ASCII 专属目录生成后加载的 `m2_user2_safe_workarea.cfg`，将四个 hart 改到未使用尾部 `0xF9000C00..0xF9000FFF`，显式 `backup=0`，且静态检查无 init/halt/resume/reset/Flash/下载命令。
  - 预检结论：`prepare_m2_user2_debug_bsp.ps1 -Apply` exit 0，schema `m2_user2_debug_bsp_v4`、结果 `USER2_HARD_TAP_DEBUG_SUPPORT_READY_TJ375_CPUTAPID_SAFE_WORKAREA_AND_RUNTIME_PC_GATE_REQUIRED`。该批同时验证 USER2/TAP=9、TJ375 CPUTAPID 覆盖、Hello map 边界、四 hart 安全 work area；串口/Programmer/OpenOCD/GDB/Flash 副作用均为零。
  - 配套 ELF 预检：`capture_m2_user2_elf_preflight.ps1` 的参数默认值同样已改为脚本体内初始化，避免 PowerShell 在 `$PSScriptRoot` 尚为空时失败。修复后 `-RequireMirror` exit 0，schema `m2_user2_elf_preflight_v2`、结果 `ELF_MIRROR_READY_FOR_USER2_RAM_LOAD`，源/ASCII 镜像完整 SHA-256 均为 `C99FD39DB437409A63A6061CD29698B5B60099B9E24A77B155B871E169BF5DA5`，副作用字段保持零。
  - 强制下一门：IDE Other options 顺序必须为：先 `-c 'set CPUTAPID 0x006A0EF3'`，随后官方 `ftdi_ti.cfg`、`debug_ti.cfg`，最后 ASCII 的 `m2_user2_safe_workarea.cfg`。仅在此配置可见且下载后 PC/反汇编为 `0xF9000000..0xF9003FFF` 时，才允许形成 CPU RAM 下载证据；仍不得 Resume 或打开 UART。缺少末尾安全 cfg、PC 异常、USER1/SoftTap/External/Flash/DDR 或连接失败均 STOP。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_163255.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_elf_preflight_20260716_163454.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_user2_profile_audit_20260716.md`、`competition_project_single_camera/tools/prepare_m2_user2_debug_bsp.ps1`、`competition_project_single_camera/tools/capture_m2_user2_elf_preflight.ps1`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 USER2 work area 加载前纠偏）
  - 适用范围：仅隔离 CPU Hello 的 OpenOCD RAM 下载前静态配置；不连接调试器、不加载或执行 ELF、不打开 UART，不关闭 `final_project/` A0/G4。
  - 根因与取代关系：上一条 v4 的 `m2_user2_safe_workarea.cfg` 是在官方 `debug_ti.cfg` 后第三个加载，因而该文件只能在原始配置已执行其 `init/halt` 后才改写 work area，存在加载时序窗口。它现记为**已作废、不得加载**。复核当前官方四-hart模板后发现其控制流有三处文本 `target create`（cpu0、SMP 循环、非 SMP 循环），三处均在各分支的 `init/halt` 前使用 `0xF9000000`。
  - 最新结论：受控脚本更新为 schema `m2_user2_debug_bsp_v5`，`-Apply` 的官方 BSP generator exit 0，结果 `USER2_HARD_TAP_DEBUG_SUPPORT_READY_TJ375_CPUTAPID_SAFE_DEBUG_CFG_AND_RUNTIME_PC_GATE_REQUIRED`。它保持源 `debug_ti.cfg`（SHA-256 `60B754D1FC0D3F38B0939D8087A481627E0AC31C39CED2A43834A48BF0F60DE2`）不变，生成 `debug_ti_m2_safe.cfg`（SHA-256 `9CAD838541A9E9F8CA489F90693AECEAA5E0CB6ED75745E7EBDA72EBD52686D5`）。独立复核为：源中重叠 work-area 3 处、派生文件中重叠 0 处/尾部 `0xF9000C00` 3 处，且派生文本仅有这三处替换；尾部区间 `0xF9000C00..0xF9000FFF` 在 Hello 已用 `0xF9000000..0xF9000A30` 之后、16 KiB RAM 之内。
  - 强制下一门：IDE 只允许按此顺序建立 TI/Titanium 硬 TAP 配置：`-c 'set CPUTAPID 0x006A0EF3'`、`ftdi_ti.cfg`、`debug_ti_m2_safe.cfg`。不得加载原始 `debug_ti.cfg` 或旧 `m2_user2_safe_workarea.cfg`。仍必须使用固定 ASCII ELF、`Debug in RAM`、image/symbol load、no manual PC、`stop at main`；下载后、任何 Resume 或 UART 前，PC/反汇编必须截图为 `0xF9000000..0xF9003FFF`。出现任一其它地址、USER1、SoftTap、External、Flash、DDR 或连接失败均 STOP。
  - 副作用边界：v5 脚本运行未调用 Programmer/OpenOCD/GDB/串口/Flash，未重新配置 FPGA；这不是 USER2 已连通、ELF 已下载、CPU 已执行或 UART0 已输出的证据。
  - 证据路径：`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_163949.json`（v5 首次严格拒绝记录）、`competition_project_single_camera/docs/debug_sessions/evidence/m2_user2_debug_bsp_20260716_164013.json`（最终 PASS）、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_user2_profile_audit_20260716.md`、`competition_project_single_camera/tools/prepare_m2_user2_debug_bsp.ps1`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_user2_ram_download_operator_card_20260716.md`。

- 日期：2026-07-16，来源 Agent：Codex（单摄 M2 UART0 只读横幅守卫离线准备）
  - 适用范围：仅为隔离 CPU Hello 的后续 UART0 监听准备 fail-closed 工具；不授权打开串口，不表示 USER2、CPU、UART0 或任何 `final_project/` A0/G4/G5–G11 Gate 已通过。
  - 最新结论：新增 `competition_project_single_camera/tools/capture_m2_uart0_banner.ps1`。它仅允许当前 FTDI 候选 COM3/COM7/COM8，115200 8N1、无流控、DTR/RTS=false，并且**默认不打开串口**。只有 Codex 后续签发的 JSON 同时绑定当前 bitstream SHA-256 `2EA4AD287CCC6BFBF113E718B59EF6F5807222AFE1ECE3D55BD1E24D37FB8347`、ELF SHA-256 `C99FD39DB437409A63A6061CD29698B5B60099B9E24A77B155B871E169BF5DA5`、结果 `M2_USER2_RAM_PC_GATE_APPROVED` 与 PC 范围 `0xF9000000..0xF9003FFF` 后，显式 `-Listen` 才可能打开一个端口。工具从不发送任何字节，也不实现单字符回显。
  - 离线验证：PowerShell AST parse 的 `PARSE_ERROR_COUNT=0`；无 `-Listen` dry-run 为 `DRY_RUN_NO_SERIAL_OPEN`，`serial_port_opened=false` / `uart_bytes_sent=0`；故意以 `-Listen` 但不给批准 JSON 时 exit 2，`HOLD_PC_GATE_APPROVAL_REQUIRED_NO_SERIAL_OPEN`，`serial_open_attempted=false` / `serial_port_opened=false` / `uart_bytes_sent=0`。两种验证均不打开 COM 口，不访问 Programmer/OpenOCD/GDB/Flash，也不改变 FPGA。
  - 强制下一门：仍先执行 USER2 RAM 下载操作卡并回传配置页、完整 Console、PC/反汇编停点截图；在 Codex 复核 PC 范围并另行签发监听操作卡前，不得运行本工具的 `-Listen`，不得打开 COM3/COM7/COM8/COM11，更不得发送 `x`、接触 UART2/J52 或机械臂。
  - 证据路径：`competition_project_single_camera/tools/capture_m2_uart0_banner.ps1`、`competition_project_single_camera/docs/debug_sessions/m2_uart0_banner_capture_preparation_20260716.md`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_banner_capture_postpatch_dryrun_20260716_164813.json`、`competition_project_single_camera/docs/debug_sessions/evidence/m2_uart0_banner_capture_postpatch_unapproved_20260716_164814.json`。
