...
- 日期：2026-07-16，来源 Agent：Codex / 用户现场确认（来源 Hard SoC bitstream USER2 + 片上 RAM + UART0 Hello 板级 Gate）
  ...
  - 下一门：先对当前可用仓库新构建 `6B6728...` 重走完全断电冷启动、JTAG SRAM临时加载、HDMI、USER2、NDMRESET、片上 RAM Hello 和UART0回显 Gate。该文件实际加载通过前不进入 APB MAGIC、feature snapshot、CPU分类主循环、OSD、按键、UART2/J52或机械臂。
