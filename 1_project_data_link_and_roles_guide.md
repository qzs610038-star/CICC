...
本指南配合单摄隔离候选工程（[competition_project_single_camera](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/)）的最新标准，详细拆解单摄 F1 阶段的系统级数据链路传输规则，并明确团队三位成员（A: libaoxun, B: wsc, C: qzs）的分工边界。
...
flowchart TB
    ...
    LIB["4. libaoxun 分图<br/>视频旁路、I1 active、OSD 硬件边界"]
...
*   **共同事实源 (Common Source of Truth)**：指项目中的权威物理边界（如 [CURRENT_STATE.md](file:///d:/第十届集创赛-雄芯院材料/CURRENT_STATE.md)、[AGENTS.md](file:///d:/第十届集创赛-雄芯院材料/AGENTS.md) 和同批编译产生的 `soc.h` 等），任何文档在描述硬件和状态时，都必须以此为最高基准。
...
|---|---|---|---|
| [1. 主图](file:///d:/第十届集创赛-雄芯院材料/learning_guides/接口对齐与数据链路学习/1_project_data_link_and_roles_guide.md) | 全员 | 全链路与 I0-I5 | 实线不是板级 PASS；虚线接口仍待 F1 ABI 审查 |
| [2. qzs 分图](file:///d:/第十届集创赛-雄芯院材料/learning_guides/接口对齐与数据链路学习/2_qzs_role_data_link_and_control_logic.md) | qzs/全员 | `CTRL → I4 → OSD`、`I5`、G1/F1 Gate | OSD 未实现；UART2/J52 与机械臂在 F1 阶段阻断 |
| [3. wsc 分图](file:///d:/第十届集创赛-雄芯院材料/learning_guides/接口对齐与数据链路学习/3_wsc_cpu_semantics_and_interface_guide.md) | wsc/全员 | `I1 → ADAPT → CLS → CTRL → I4` | Host/fake transport 不是真实 MMIO；不稳定快照不产生稳定业务结论 |
| [4. libaoxun 分图](file:///d:/第十届集创赛-雄芯院材料/learning_guides/接口对齐与数据链路学习/4_libaoxun_fpga_video_and_feature_interface_guide.md) | libaoxun/全员 | `视频主链 → Tap → I1`、`I2 → active config → Tap` | Tap 只读不回压；当前顶层 feature 仍禁用且未接 CPU |

### 全队每次接口变更前必须一起核对

| 要确认的问题 | 共同文件 | 谁给出结论 |
|---|---|---|
| 当前 Gate、禁止项和板级证据边界是什么？ | [CURRENT_STATE.md](file:///d:/第十届集创赛-雄芯院材料/CURRENT_STATE.md) 与当前批次操作卡 | qzs 组织，三人确认 |
| I1/I2/I3/I4/I5 的语义、状态和所有权是什么？ | [F1_INTERFACE_ALIGNMENT_DRAFT.md](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/F1_INTERFACE_ALIGNMENT_DRAFT.md) | 三人共同确认 |
| 特征字段、flags、ACK/overrun、I2 active 规则是什么？ | [single_camera_feature_contract.md](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/single_camera_feature_contract.md) | libaoxun + wsc |
...
flowchart TB
    ...
    subgraph F1["F1 待冻结业务接口：WIRE ABI / CDC / 板级证据均未完成"]
        ...
        I2_COMMIT[["CFG_COMMIT(config_seq)"]]
        I2_ACTIVE[["active config<br/>仅在 VSYNC 边界整组生效"]]
        I4[["I4 结果语义<br/>round_id + frame_id + decision + reason"]]
    end
...
*   **OSD (On-Screen Display)**：屏幕字符显示叠加器，负责在 HDMI 输出的画面上绘制目标红框、文字提示（如分类结果和理由）。
...
1.  **A区（流水线前端）**：摄像头是入库货物扫描口，把源源不断的集装箱（像素流）源源不断送往 HDMI 这个“货物出港大门”。在传送带旁，libaoxun 装了一台只读的“高速拍照检测仪（Tap）”，在集装箱快速通过时疯狂按快门拍照并汇总出各种货物的颜色和尺寸数据，但绝不减慢传送带的速度。
2.  **F1 调度层（数据暂存柜）**：拍下的集装箱快照（特征数据）被安全放入“高防盗货物柜（I1 快照发布与 APB 总线）”，并且设置了“取货密码（frame_id 校验与 Valid-ACK）”。在柜台另一侧，指挥官的参数通过“暂存邮箱（I2 暂存-提交区）”保存，只有在流水线每次停工重整时（VSYNC 场消隐）才统一调入使用。
3.  **B区（调度大脑）**：wsc 作为大脑指挥官，轮询检查货物柜。一旦发现密码正确（双读 frame_id 一致），就让“分类器”和“状态机”判断货物颜色和尺寸，确定是哪一轮任务，制定该装卸还是该跳过。
4.  **C区（执行与安全防线）**：qzs 负责在大脑决策出来后，指挥中控屏幕（OSD 渲染器）用人话（文字理由）在大屏幕上向全场广播今天分拣的结果。此外，为了防止自动化起重机（myCobot）在调试期间失控砸坏现场（异常暴冲），qzs 在物理上彻底拔掉了起重机的电控线，并让大脑指挥官强行下达“原地待命（arm_enabled=0）”的锁死指令。
...
*   **线 ABI 状态**：寄存器偏移未冻结，基于 [single_camera_feature_adapter.h](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/cpu/include/single_camera_feature_adapter.h) 进行语义提取。
*   **特征定义**：
...
*   **功能**：CPU 识别结果（颜色、形状、尺寸关系、动作 `decision`、跳过/抓取理由 `reason`）回写到 OSD 寄存器，由 FPGA 叠加渲染在 HDMI 上。**禁止直接显示不可解释的寄存器裸值**。
...
*   **物理状态**：UART2 串口引脚（1000000 bps）在硬件上拔除，软件强制 `arm_enabled = 0`，且仅允许发出 `EXECUTE_ARM_DISABLED` 决策，禁止发送任何 myCobot 控制帧。此为安全防线。

<details>
<summary>💡 点击展开/收起：I5 接口生动形象中文介绍</summary>

这就像是**“核弹发射台的物理隔离线与软件硬置零锁”**。机械臂是一个物理动能极大的铁疙瘩，如果上电初始化或总线逻辑有误，串口发送了乱码，机械臂就会像疯子一样狂转暴冲，砸坏现场。我们在 F1 阶段拉起了最野蛮但也最绝对的安全防线：软件上在大脑里把动作授权硬写死为 0（绝对不出臂），物理上直接把连接电控串口的线缆全部拔掉。双锁合璧，绝对安全。
...
*   [ ] **双读校验语义**：在 [single_camera_feature_contract.md](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/single_camera_feature_contract.md) 与 [single_camera_runtime.c](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/cpu/src/single_camera_runtime.c) 中，是否保持“读前后 `frame_id` 一致才消费、同帧才 ACK”的语义？真实 MMIO backend 当前仍 fail-closed。
