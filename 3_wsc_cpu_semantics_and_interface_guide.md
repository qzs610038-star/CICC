...
本指南是 [主图：项目数据链路与三人分工](file:///d:/第十届集创赛-雄芯院材料/learning_guides/接口对齐与数据链路学习/1_project_data_link_and_roles_guide.md#2-项目整体数据传输链路与队员分工图示) 的 **CPU 细节补充**。它放大主图中的 `I1 → ADAPT → CLS → CTRL → I4` 支路，并说明 CPU 对 I2、I3、I0 的边界。
...
事实源优先级： [CURRENT_STATE.md](file:///d:/第十届集创赛-雄芯院材料/CURRENT_STATE.md) → [F1 接口对齐总账](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/F1_INTERFACE_ALIGNMENT_DRAFT.md) → [单摄特征快照契约](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/single_camera_feature_contract.md) → 同批 CPU 源码与测试。
...
flowchart TB
    ...
    subgraph MAIN_PIECE["【主路线图对应片段】(1号总图中的 CPU 内部处理链路)"]
        M_I1[["I1 快照"]] -.-> M_ADAPT["ADAPT<br/>(适配器)"] --> M_CLS["CLS<br/>(分类器)"] --> M_CTRL["CTRL<br/>(状态机)"]
        M_CTRL -.->|I4 结果| M_I4[["I4 寄存器"]]
        M_CTRL -.->|I2 配置| M_I2[["I2 寄存器"]]
    end
    ...
    subgraph DETAIL_LOGIC["【wsc 负责的特征校验、分类与状态机处理细致逻辑】"]
        ...
        I4_STG[["I4 暂存区：写入 decision 与理由"]]
        I2_STG[["I2 暂存区：调节 ROI 与过滤阈值"]]
    end
...
4.  **决策（CTRL）**：海关总长（CTRL 控制器）翻开红头文件（I3 任务指示）。如果是“指定抓取红色正方体”，总长一拍桌子写下“目标命中（reason=TARGET_MATCH）”和“同意放行”，但因为安全防线限制，大印上依然盖着“原地待命（arm_enabled=0）”，最后把战报送到电子液晶屏（I4）上通告全场。
...
I2 配置是 CPU 向硬件下达的**“滤镜与侦察区域校准信”**。大脑需要经常根据环境光调整 ROI 坐标和三色过滤门限。大脑在发信时，不能写一个字寄出一页，而是把调校参数写在一张 staging 暂存信纸上。最后在信封写上递增的版本号并敲下 `CFG_COMMIT` 提交大印。FPGA 只有在场消隐消退边界才会整体读这封信，这确保了前线士兵在工作时，图像特征的提取绝不会因为调焦指令中途改变而发生撕裂。
...
I4 结果是 CPU 决策完后写出的**“战报回折单”**。它必须是一份人人都看得懂的可解释报告：“轮次为第 1 轮”、“已处理第 24 帧快照”、“判定结果为蓝色圆柱体”、“做出了跳过（SKIP）决策，因为颜色与考题不符”。大印上安全指示灯 `arm_enabled` 强制写死为 0。这份单据被完整地塞入 OSD staging 暂存箱，并拉起有效标志，交给 OSD 渲染器上屏广播。
...
这条顺序对应 [single_camera_feature_adapter.h](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/cpu/include/single_camera_feature_adapter.h) 和 [single_camera_runtime.c](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/cpu/src/single_camera_runtime.c)。其中 real MMIO backend 当前故意全部返回不可用，正是为了阻止未冻结地址被误用。
...
|---|---|---|
| libaoxun | [single_camera_feature_contract.md](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/single_camera_feature_contract.md)、[feature_stats_tap.v](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/src/feature_stats/feature_stats_tap.v)、同批 `soc.h` | “这一帧的字段/flags/`config_seq` 是什么；何时发布；CPU 如何对同帧 ACK；尚无何种 ABI。” |
| qzs | [F1_INTERFACE_ALIGNMENT_DRAFT.md](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/integration/F1_INTERFACE_ALIGNMENT_DRAFT.md)、[2_qzs_role_data_link_and_control_logic.md](file:///d:/第十届集创赛-雄芯院材料/learning_guides/接口对齐与数据链路学习/2_qzs_role_data_link_and_control_logic.md) | “I4 只输出可解释语义，I5 仍无 transport，何种 Gate 后才能改。” |
| 全队 | [single_camera_feature_adapter.c](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/cpu/src/single_camera_feature_adapter.c)、[single_camera_f1.c](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/cpu/src/single_camera_f1.c)、[single_camera_runtime.c](file:///d:/第十届集创赛-雄芯院材料/competition_project_single_camera/cpu/src/single_camera_runtime.c) | “CPU 拒绝何种输入、何时产生结果、何时 ACK、何时只等待。” |
| 全队 | [CURRENT_STATE.md](file:///d:/第十届集创赛-雄芯院材料/CURRENT_STATE.md) 与当前批次 G1 操作卡 | “当前只是离线/Host 证据，任何板级动作必须遵守 Checkpoint 与禁止项。” |
