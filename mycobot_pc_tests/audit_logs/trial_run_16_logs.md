# 第十六次试运行（2026-07-07 V2.5 异步平滑过渡首测与软到位不收敛诊断）

> 脚本: `mycobot_pc_tests/teach_replay_pick_return_ready.py` (V2.5)
> 方案: `mycobot_pc_tests/audit_logs/return_ready_teach_replay_v2_plan.md` §14.8 (V2.5)
> 上一轮 (Run-15): 验证 V2.4 提速成果，回零物理耗时压缩至 1.2s，确认微调超时和去冗余成功。
> 本轮状态: **测试通过但提速未达预期**。平滑手势过渡异步流和异步软到位逻辑已成功执行，但第二阶段和第三阶段末段均因 `max_diff=1.31°` 触发 2.5s 软超时，最终回退走阻塞 sync 兜底，导致 step 10+11 耗时回升至 5.0s。
...
当前稳定关节角: [9.31, 7.03, -0.43, -4.65, 0.52, 19.59]
相对 HOME_ANGLES 全轴偏差: [9.31, 7.03, 0.43, 4.65, 0.52, 19.59]
大臂 1-5 轴偏差: [9.31, 7.03, 0.43, 4.65, 0.52], arm_max_diff=9.3
第 6 轴末端旋转偏差: 19.6
arm_max_diff=9.3 <= 45.0，回直立零位...
  -> [V2.5 方向1] step11回零: 异步回零 send_angles(HOME, speed=30)，软到位循环 (tol=1.0°, timeout=2.5s)...
  -> [V2.5 方向1] step11回零: 软超时未收敛（max_diff=1.31°），阻塞 sync_send_angles 收尾兜底 (speed=25 [V2.4已验证], timeout=12s)...
  -> [V2.5 方向1] step11回零: 收尾 sync_send_angles 返回 1
  -> [V2.5 方向1] step11回零: sync 收尾后 max_diff=1.31°。
...
10+11. 末段回零：drop_hover -> home_ready -> HOME...
  ...
  -> [V2.5 方向2] HOME 软超时未收敛（max_diff=1.31°），阻塞 sync_send_angles  收尾兜底 (speed=25 [V2.4已验证], timeout=12s)...
  -> [V2.5 方向2] 收尾 sync_send_angles 返回 1
  -> [V2.5 方向2] sync 收尾后 max_diff=1.31°。
  -> [V2.2 耗时] step 10+11 (平滑过渡): 5.0s
...
从上述终端日志可以清晰看到，两次回零操作（第二阶段安全自检回零和第三阶段过渡回零）的耗时未能按预期大幅缩短，关键在于：

1.  **软到位超时未收敛**：
    *   第二阶段自检回零中，程序运行了 2.5s 软到位检测，打印了 `软超时未收敛（max_diff=1.31°）`，随后不得不调用 $25$ 速度的阻塞 `sync_send_angles` 收尾。
    *   第三阶段过渡回零（Step 10+11）中，虽然成功在 $1.84^\circ \le 5^\circ$ 时提前下发了 HOME 目标，但此后在 HOME 的异步软到位轮询阶段，**同样卡在了 `max_diff=1.31°` 超出软到位容差 `HOME_RETURN_ASYNC_SOFT_TOL = 1.0` 的门限**，导致空等了 2.5s 软超时后，又一次退回阻塞的 `sync_send_angles` 收尾，导致合并后的 Step 10+11 总耗时依然高达 $5.0\text{s}$。
2.  **根因归结**：
    由于 myCobot 280 电机物理回正的精度死区及回弹特性，其实际物理零位角度读取与绝对零位 `[0,0,0,0,0,0]` 的最大轴偏差常年稳定在 **$1.1^\circ \sim 1.4^\circ$**。将软到位判定阈值设为过于苛刻的 **$1.0^\circ$** 导致判定程序绝无可能在非阻塞的轨迹惯性下触发判定退出，直接废掉了整个异步软到位的提速增益，甚至产生了空等 2.5s 软超时的反向负面效益。
...
1.  **放宽软到位阈值（主要提速杠杆）**：
    将 `HOME_RETURN_ASYNC_SOFT_TOL` 从当前的 `1.0` 放宽至 **`1.5`**（或为了鲁棒性放宽至 `2.0`）。直立零位附近多偏 $1.5^\circ$ 在物理层面上完全竖直，绝无干涉风险。只要调整此阈值，系统在运动约 $0.5\text{s}$、角差落入 $1.31^\circ$ 时即会立刻判定收敛退出，免去 $2.5\text{s}$ 空等和阻塞收尾，Step 10+11 耗时预期将暴降至 **$1.5 \sim 2.0\text{s}$**。
2.  **微调软超时时间**：
    将 `HOME_RETURN_ASYNC_TIMEOUT` 从 `2.5s` 压低至 **`1.2s` 或 `1.5s`**。若运动 1.2s 都无法逼近零位，基本可断定有阻挡或机械异常，不需要空等 2.5s，这样可进一步降低未收敛情况下的兜底等待开销。
