...
typedef enum {
    SC_SHAPE_UNKNOWN = 0,
    SC_SHAPE_CUBE = 1,
    SC_SHAPE_CYLINDER = 2, /* Uncalibrated heuristic — treated as NON_CUBE in production. */
    SC_SHAPE_CONE = 3      /* Uncalibrated heuristic — treated as NON_CUBE in production. */
} sc_shape_t;
...
/* Production-reliable shape gate: only CUBE is trusted for task decisions in phase A.
   ...
   UNKNOWN also keeps the controller in WAIT/ACQUIRING. */
