...
$output = [ordered]@{
    ...
    manifest_a = [ordered]@{path=$a.Path;sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $a.Path).Hash}
    manifest_b = [ordered]@{path=$b.Path;sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $b.Path).Hash}
