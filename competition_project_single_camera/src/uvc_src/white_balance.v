module white_balance #(
    ...
    input  wire                        vs_in,
    input  wire                        de_in,
    // Debayer provides two pixels per clock as {B,G,R,B,G,R}.
    input  wire [DATA_WIDTH*6-1:0]     data_in,
    output reg                         hs_out,
    output reg                         vs_out,
    output reg                         de_out,
    ...
    wire [DATA_WIDTH-1:0] odd_b_in  = data_in[DATA_WIDTH*6-1:DATA_WIDTH*5];
    wire [DATA_WIDTH-1:0] odd_g_in  = data_in[DATA_WIDTH*5-1:DATA_WIDTH*4];
    wire [DATA_WIDTH-1:0] odd_r_in  = data_in[DATA_WIDTH*4-1:DATA_WIDTH*3];
    wire [DATA_WIDTH-1:0] even_b_in = data_in[DATA_WIDTH*3-1:DATA_WIDTH*2];
    wire [DATA_WIDTH-1:0] even_g_in = data_in[DATA_WIDTH*2-1:DATA_WIDTH];
    wire [DATA_WIDTH-1:0] even_r_in = data_in[DATA_WIDTH-1:0];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Keep HDMI-facing pixels in {R,G,B,R,G,B} order.
            data_out <= {
